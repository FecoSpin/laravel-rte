...
<body>
    ...
    @if(env('APP_DEBUG'))
    <div style="page-break-before: always; padding: 20px; background: #f8f9fa; border: 1px solid #ddd; margin-top: 20px; font-family: monospace;">
        <h3>🔍 Datos de depuración</h3>
        
        <h4>📋 Variables disponibles en la vista:</h4>
        <ul>
            <li><strong>$formulario</strong> ({{ get_class($formulario) }})</li>
            <li><strong>$profile</strong> ({{ isset($profile) ? get_class($profile) : 'No definida' }})</li>
        ...
        <pre>{{ json_encode($formulario->toArray(), JSON_PRETTY_PRINT) }}</pre>
        
        <h4>📚 Campos Formativos ({{ $formulario->camposFormativos->count() }}):</h4>
        @foreach($formulario->camposFormativos as $campo)
            <div style="margin-bottom: 15px; border-bottom: 1px solid #ddd; padding-bottom: 10px;">
                <strong>Campo:</strong> {{ $campo->nombre }}
                <pre>{{ print_r($campo->toArray(), true) }}</pre>
                @if($campo->grados->isNotEmpty())
                    <strong>Grados:</strong>
                    <pre>{{ print_r($campo->grados->toArray(), true) }}</pre>
                @else
                    <p>No hay grados asociados</p>
                @endif
            </div>
        @endforeach
        
        <h4>🚀 Proyectos Colaborativos ({{ $formulario->proyectosColaborativos->count() }}):</h4>
        @if($formulario->proyectosColaborativos->isNotEmpty())
            <pre>{{ json_encode($formulario->proyectosColaborativos->toArray(), JSON_PRETTY_PRINT) }}</pre>
        @else
            <p>No hay proyectos colaborativos registrados</p>
        @endif
        
        <h4>📝 Relaciones cargadas:</h4>
        <ul>
            <li>camposFormativos: {{ $formulario->relationLoaded('camposFormativos') ? '✅' : '❌' }}</li>
            <li>proyectosColaborativos: {{ $formulario->relationLoaded('proyectosColaborativos') ? '✅' : '❌' }}</li>
            <li>cursosEnLinea: {{ $formulario->relationLoaded('cursosEnLinea') ? '✅' : '❌' }}</li>
            <li>usuario: {{ $formulario->relationLoaded('usuario') ? '✅' : '❌' }}</li>
            @if($formulario->relationLoaded('usuario'))
                <li>usuario.profile: {{ $formulario->usuario && $formulario->usuario->relationLoaded('profile') ? '✅' : '❌' }}</li>
            @endif
        </ul>
    </div>
    @endif
